...
import { REDPILL } from "./commands/redpill";
import { BLUEPILL } from "./commands/bluepill";
...
//WRITELINESCOPY is used during the "clear" command
...
const COMMANDS = ["help", "about", "projects", "experience", "whoami", "redpill", "bluepill", "repo", "banner", "clear"];
...
function commandHandler(input : string) {
  ...
  switch(input) {
    ...
    case 'banner':
      if(bareMode) {
        writeLines(["Nipun's MainFrame v7.27.04", "<br>"])
        break;
      }
      setTimeout(() => {
    ...
    case 'redpill':
      if(bareMode) {
        writeLines(["You already know too much.", "<br>"])
        break;
      }
      writeLines(REDPILL);
      setTimeout(() => {
        window.open(command.social.resume, '_blank');
      }, 1500);
      break;
    case 'bluepill':
      if(bareMode) {
        writeLines(["There's nothing left to forget.", "<br>"])
        break;
      }
      writeLines(BLUEPILL);
      setTimeout(() => {
